...
func (ctx *SubstContext) Process(mkline *MkLineImpl) {
	switch {
	case mkline.IsEmpty():
		ctx.Finish(mkline)
...
func (ctx *SubstContext) Varassign(mkline *MkLineImpl) {
	if trace.Tracing {
...
func (ctx *SubstContext) Directive(mkline *MkLineImpl) {
	if ctx.id == "" {
		return
	}
...
func (ctx *SubstContext) Finish(mkline *MkLineImpl) {
	if ctx.id == "" {
		return
	}
...
func (*SubstContext) dupString(mkline *MkLineImpl, pstr *string, varname, value string) {
	if *pstr != "" {
...
func (*SubstContext) dupBool(mkline *MkLineImpl, flag *bool, varname string, op MkOperator, value string) {
...
func (ctx *SubstContext) suggestSubstVars(mkline *MkLineImpl) {

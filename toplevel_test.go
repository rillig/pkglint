...
func (s *Suite) Test_CheckdirToplevel__recursive(c *check.C) {
	...
	t.CheckOutputLines(
		...
		"1 warning found.",
		"(Run \"pkglint -e -Wall -r ~\" to show explanations.)")
}
...
func (s *Suite) Test_CheckdirToplevel__recursive_inter_package(c *check.C) {
	...
	t.CheckOutputLines(
		...
		"2 warnings found.",
		"(Run \"pkglint -e -Wall -Call -r ~\" to show explanations.)")
}
...
func (s *Suite) Test_CheckdirToplevel__indentation(c *check.C) {
	...
	t.CheckOutputLines(
		...
		"(Run \"pkglint -fs -Wall ~\" to show what can be fixed automatically.)",
		"(Run \"pkglint -F -Wall ~\" to automatically fix some issues.)")
}

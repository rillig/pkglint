...
func (s *Suite) Test_InitVartypes__enumFrom(c *check.C) {
	...
	vartype := func(varname string) string { return mklines.mklines[1].VariableType(varname).String() }

	t.SetupVartypes()

	c.Check(vartype("EMACS_VERSIONS_ACCEPTED"), equals, "ShellList of enum: emacs31 ")
	c.Check(vartype("PKG_JVM"), equals, "enum: jdk16 openjdk7 openjdk8 oracle-jdk8 sun-jdk6 sun-jdk7 ")
	c.Check(vartype("USE_LANGUAGES"), equals, "ShellList of enum: ada c c++ c++03 c++0x c++11 c++14 c99 fortran fortran77 gnu++03 gnu++0x gnu++11 gnu++14 java obj-c++ objc ")
}

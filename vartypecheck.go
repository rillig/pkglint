...
func (cv *VartypeCheck) DependencyWithPath() {
	...
	if m, pattern, relpath, pkg := match3(value, `(.*):(\.\./\.\./[^/]+/([^/]+))$`); m {
		...
		cv.MkLine.CheckVartypePrimitive(cv.Varname, CheckvarDependency, cv.Op, pattern, cv.MkComment, cv.ListContext, cv.Guessed)
		return
	}
...
func (cv *VartypeCheck) FetchURL() {
	cv.MkLine.CheckVartypePrimitive(cv.Varname, CheckvarURL, cv.Op, cv.Value, cv.MkComment, cv.ListContext, cv.Guessed)
...
func (cv *VartypeCheck) Pathlist() {
	if !contains(cv.Value, ":") && cv.Guessed {
		cv.MkLine.CheckVartypePrimitive(cv.Varname, CheckvarPathname, cv.Op, cv.Value, cv.MkComment, cv.ListContext, cv.Guessed)
		return
	}
...
func (cv *VartypeCheck) PkgOptionsVar() {
	cv.MkLine.CheckVartypePrimitive(cv.Varname, CheckvarVarname, cv.Op, cv.Value, cv.MkComment, false, cv.Guessed)
	...
	for i := 0; i < ntokens; i++ {
		...
		case token == "-e":
			if i+1 < ntokens {
				...
				mkline.CheckVartypePrimitive(cv.Varname, CheckvarSedCommand, cv.Op, tokens[i], cv.MkComment, cv.ListContext, cv.Guessed)
			} else {

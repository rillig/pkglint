...
func (cv *VartypeCheck) homepageFtp() {
	if cv.MkLine.HasRationale("ftp", "FTP", "http", "https", "HTTP") {
		return
	}
	if hasPrefix(cv.Value, "ftp://") {
		cv.Warnf("An FTP URL does not represent a user-friendly homepage.")
		cv.Explain(

...
sub lines_logWarning($$$) {
	...
	if (lineno <= $#{lines}) {
		lines.[lineno].logWarning(msg)
	} else {
		logWarning(lines.[0].fname, "EOF", msg)
	}
}
...
sub expect_empty_line($$) {
	...
	if (expect(lines, lineno_ref, qr"^$")) {
		...
		opt_warn_space and lines.[${lineno_ref}].logNote("Empty line expected.")
		return false
	}
}
...
sub expect_text($$$) {
	...
	rv or lines_logWarning(lines, ${lineno_ref}, "Expected \"${text}\".")
	return rv
}
...
sub expect_re($$$) {
	...
	rv or lines_logWarning(lines, ${lineno_ref}, "Expected text matching re.")
	return rv
}
...
sub checkline_spellcheck($) {
	...
	if (line.text =~ m"existant") {
		line.logWarning("The word \"existant\" is nonexistent in the m-w dictionary.")
...
sub checkline_mk_varuse($$$$) {
	...
	opt_debug_trace and line.logDebug("checkline_mk_varuse(\"${varname}\", \"${mod}\", ".context.to_string().")")
	...
	if (defined(type) && !(type.is_guessed)) {
		...
		opt_warn_extra and line.logWarning("${varname} is used but not defined. Spelling mistake?")
	}
	...
	if (opt_warn_perm) {
		...
		if (is_load_time && !is_indirect) {
			line.logWarning("${varname} should not be evaluated at load time.")
			line.explain_warning(
...
"or regular expressions are modified in a subtle way.")
		...
		if (is_load_time && is_indirect) {
			line.logWarning("${varname} should not be evaluated indirectly at load time.")
			line.explain_warning(
...
"properly defined.")
		...
		if (perms !~ m"p" && perms !~ m"u") {
			line.logWarning("${varname} may not be used in this file.")
		}
	}
...
"	CONFIGURE_ENV+= --with-datafiles=\${PREFIX}/share/battalion")
	...
	if (context.shellword == VUC_SHELLWORD_FOR) {
		if (!defined(type)) {
			...
			line.logWarning("The variable ${varname} should not be used in .for loops.")
			line.explain_warning(
	...
	if (opt_warn_quoting && context.shellword != VUC_SHELLWORD_UNKNOWN && needs_quoting != dont_know) {
		...
		if (mod eq ":M*:Q" && !need_mstar) {
			...
			if (context.shellword == VUC_SHELLWORD_PLAIN) {
				line.logWarning("Please use \${${varname}${correct_mod}} instead of \${${varname}${mod}}.")
				#line.replace("\${${varname}}", "\${${varname}:Q}")
			} else {
				line.logWarning("Please use \${${varname}${correct_mod}} instead of \${${varname}${mod}} and make sure the variable appears outside of any quoting characters.")
			}
...
"\t\techo \${WORD1}\${WORD2} # still 1 word")
			...
			if (needs_quoting == false) {
				line.logWarning("The :Q operator should not be used for \${${varname}} here.")
				line.explain_warning(@expl)
			} elsif (needs_quoting == doesnt_matter) {
				line.logNote("The :Q operator isn't necessary for \${${varname}} here.")

...
func (s *Suite) Test_MkExprChecker_warnPermissions__not_directly_and_no_alternative_files(c *check.C) {
	...
	t.CheckEquals(apiDependsType.String(), "PackagePattern (list, package-settable)")
...
func (s *Suite) Test_MkExprChecker_checkToolsPlatform(c *check.C) {
	...
	mklines := t.NewMkLines("filename.mk",
		...
		"CONFIGURE_ENV.any+=\tA=${TOOLS_PLATFORM.undefined:Q}")
	...
	t.CheckOutputLines(
		...
		"WARN: filename.mk:29: TOOLS_PLATFORM.undefined is undefined on SunOS.")
}

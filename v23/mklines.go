...
func (mklines *MkLines) collectVariable(mkline *MkLine, infrastructure bool, addToUseTools bool) {
	...
	case "PLIST_VARS":
		for _, id := range mkline.ValueFields(resolveExprs(mkline.Value(), mklines, nil)) {
			if trace.Tracing {
	...
	for _, mkline := range mklines.mklines {
		...
		if mkline.IsInclude() && mkline.IncludedFile() == "../../mk/bsd.options.mk" {
			...
			for _, opt := range mkline.ValueFields(optional) {
				supported.append(mklines.allVars.LastValue("PKG_OPTIONS_GROUP." + opt))
			}
...
func (mklines *MkLines) ExpandLoopVar(varname string) []string {
	...
	for i := len(mklines.indentation.levels) - 1; i >= 0; i-- {
		...
		resolved := resolveExprs(mkline.Args(), mklines, nil)

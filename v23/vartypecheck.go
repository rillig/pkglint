...
func (cv *VartypeCheck) DependencyWithPath() {
	...
	cv.WithValue(pattern).PackagePattern()
}
...
func (cv *VartypeCheck) LdFlag() {
	...
	if m, rpathFlag := match1(ldflag, `^(-Wl,--rpath,|-Wl,-rpath-link,|-Wl,-rpath,|-Wl,-R\b)`); m {
		cv.Warnf("Use ${COMPILER_RPATH_FLAG} instead of %q.", rpathFlag)
		return
	}
...
func (cv *VartypeCheck) PackagePattern() {
	ck := PackagePatternChecker{cv.Varname, cv.MkLine, cv.MkLines}
	ck.Check(cv.Value, cv.ValueNoVar)
}
